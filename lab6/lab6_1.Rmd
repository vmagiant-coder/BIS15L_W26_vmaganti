...
ggplot(data = penguins,
  ...
  labs(title="Body Mass vs. Flipper Length Amongst Various Species", x = "Body Mass", y = "Flipper Length") +
  geom_smooth(method="lm", se=F)
```

#if there is an error on the midterm/couldn't finish a problem and you are trying to knit it, put a "#" on the half-completed code and then knit it since R won't let you knit without the code being correct !!
...
```
#keep all = kept all the other variables for one of the matches
...
mammals <- mammals %>%
  clean_names()
```
#good practice to add clean names into beginning to clean the file 
...
mammals %>%
select(genus, species, newborn) %>%
  mutate(newborn_new=ifelse(newborn == -999.00, NA, newborn)) #giving newborn a new name, if newborn is -999 make it NA, if NA, make it newborn
```
...
mammals %>%
  ...
  ggplot(data = mammals,
         mapping=aes(x=gestation, y=newborn)) +
           geom_point() +
...
mammals %>%
  ...
  ggplot(data = mammals,
         mapping=aes(x=mass, y=max_life)) +
  geom_point() +
  scale_y_log10() +
  scale_x_log10() +
  labs(title = "Mammal Size vs. Lifespan", x= "Mammal Size", y = "Lifespan") +
geom_smooth(method = lm, se = F)
```
