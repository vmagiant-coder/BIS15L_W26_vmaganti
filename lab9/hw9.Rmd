...
sydneybeaches_long %>%
  ...
  group_by(Site) %>%
  summarize(
  avg_enterococci = mean(`Enterococci (cfu/100ml)`, na.rm = T)
  ) %>%
  arrange(desc(avg_enterococci)) %>%
  slice_head(n=1)
```

On average, the beach with the highest pollution in 2013 was Little Bay Beach with an Enterococci value of 121.5345 cfu/100ml.
...
Malabar Beach has the highest overall average pollution.
