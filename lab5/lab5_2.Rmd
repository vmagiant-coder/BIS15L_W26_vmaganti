...
```{r}
fish <- read.csv("/Users/magantiv/Desktop/clone_repository_github/BIS15L_W26_vmaganti/lab5/data/Gaeta_etal_CLC_data.csv")
names(fish)
```
...
```
'slice()' #pull out observations in top and bottom
```{r}
penguins %>%
  select(species, body_mass_g) %>%
  filter(species=="Gentoo") %>%
slice_max(body_mass_g, n=1) #max value pulled out, don't forget to specify variable
```
```{r}
penguins %>%
  select(species, body_mass_g) %>%
  filter(species=="Gentoo") %>%
slice_min(body_mass_g, n=1) #minimum value will be pulled out
```
