...
ggplot(data=penguins,
       mapping=aes(x=body_mass_g, y=flipper_length_mm))+
  geom_point(na.rm = T)+ #na.rm = T makes the missing value note go away
  geom_smooth(method=lm, se=T) #gives the line of best fit
```
...
ggplot(data=penguins,
  ...
  geom_point(mapping=aes(color=bill_length_mm))+ #can do, not always necessary
  geom_smooth(method=lm, se=F)+
  labs(title="Bill Length (mm) vs Bill Depth (mm)", x= "Bill Length (mm)", y= "Bill Depth (mm)")
```
...
```{r}
ggplot(data=penguins,
       mapping=aes(x=species))+ #don't need to add y, it will calculate for us
  geom_bar() #one type of bar plot
```
...
ggplot(data=penguins,
       mapping=aes(x=species))+
  geom_bar(mapping=aes(fill=species)) #use fill to fill the bars instead of just outlining the bars
```
